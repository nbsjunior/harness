...
todd agent:run -a copilot -p "Review the PR diff for security issues"

# Parallel fan-out
todd agent:fanout -a copilot,claude -p "Summarize risks in src/auth/"

# Spec suggestions from repo layout
todd spec:discover --write
```

## Tips

- Set `TODDSPECT_WORKSPACE` to `$GITHUB_WORKSPACE`
- Use `todd check getGoat --json` in a prior step to fail fast when agents are misconfigured
