...
| **Agent** | **Local** by default when a Cursor API key is set | Yes — edits `TODDSPECT_WORKSPACE` |
| **Spec+Agent** | Like Agent + `.toddspect/specs/` injected | Yes |
...
1. **Cursor API key** — [cursor.com/dashboard/integrations](https://cursor.com/dashboard/integrations) or `CURSOR_API_KEY` / `toddspect.connectors.cursor.apiKey`
2. VS Code setting **`toddspect.cursor.agentExecution`**: `auto` (default) or `local`

**Does not use GitHub Copilot quota** for this path — you do not need `gh auth login` for Cursor local Agent.

## Setting: `toddspect.cursor.agentExecution`
...
| VS Code setting | `toddspect.cursor.agentExecution` in `packages/extension/package.json` |
