...
| **Spec auto-discovery** | Implemented | `toddspect spec:discover` / IPC `spec:discover` |
| **Multi-agent fan-out** | Implemented | `toddspect agent:fanout -a copilot,claude -p "..."` |
...
| **Web UI (remote)** | Implemented (MVP) | `toddspect web:serve` → http://127.0.0.1:3847 |
