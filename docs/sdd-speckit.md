...
ToddSpect aligns with [GitHub spec-kit](https://github.com/github/spec-kit) while keeping ToddSpect-native specs in `.toddspect/specs/` (Skills, Tools, Workflows for `spec+agent` chat).

## Two layers

| Layer | Path | Purpose |
|-------|------|---------|
| **ToddSpect specs** | `.toddspect/specs/` | Reusable agent Skills/Tools/Workflows (YAML/Markdown) |
| **spec-kit SDD** | `.toddspect/sdd/` | Full product workflow: constitution → spec → plan → tasks → implement |

## spec-kit commands in ToddSpect

| spec-kit | ToddSpect SDD UI | Artifact |
|----------|----------------|----------|
...
## What ToddSpect adds beyond spec-kit
...
- `toddspect spec:discover` for repo-based ToddSpect spec suggestions
...
ToddSpect `.toddspect/sdd/` can coexist with `.specify/` from `specify init`.

## Related docs

- [sdd-specs.md](sdd-specs.md) — ToddSpect Skills/Tools/Workflows (`.toddspect/specs/`)
