...
Todd of AIDLC runs every chat message through a **prompt optimization pipeline** before routing to Copilot, Cursor, Claude, Devin, or Kiro. The goal is **lower token use** (efficiency) and **more consistent, on-task answers** (quality) — without you rewriting prompts by hand.
...
| **Merge system guidance** | One Todd of AIDLC guidance block instead of many | Fewer duplicate system tokens |
...
When optimization is on, Todd of AIDLC prepends a **system message** (unless already present) that applies to **every** provider:
...
- Spec+Agent: active Todd of AIDLC specs are authoritative
...
See also [user-manual.md](user-manual.md) and [why-todd-of-aidlc.md](why-todd-of-aidlc.md).
